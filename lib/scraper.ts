...
export async function scrapeTodayTenders(_positiveKeywords?: string[], targetDate?: string): Promise<{ tenders: ScrapedTender[], rawHtml: string }> {
  try {
    ...
    const PAGE_SIZE = 100
    const MAX_PAGES = 100 // 安全上限：最多抓 10,000 筆
    ...
    while (pageIndex <= MAX_PAGES) {
      ...
      const pageTenders: ScrapedTender[] = []
      ...
      console.log(`[Scraper] Page ${pageIndex}: ${pageTenders.length} tenders (total: ${allTenders.length})`)

      // 取回筆數少於 pageSize，代表已到最後一頁
      if (pageTenders.length < PAGE_SIZE) break
    ...
    return {
      tenders: allTenders,
      rawHtml: `Pagination complete. ${pageIndex} pages fetched, ${allTenders.length} total tenders.`
    }
  } catch (error: any) {
